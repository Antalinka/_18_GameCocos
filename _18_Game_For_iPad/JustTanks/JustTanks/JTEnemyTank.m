...
-(id) initWithSprite:(CCSprite *)sprt scene:(JTGameScene *)scene properties:(NSDictionary *)props{
    if (self = [super initWithSprite:sprt scene:scene properties:props]) {
        ...
        self.position = self.startPos = CGPointFromString([props objectForKey:key_position]);
        self.health = self.allHealth =  [[props objectForKey:key_health]intValue];
        self.damage = [[props objectForKey:key_damage]intValue];
        self.moveSpeed = [[props objectForKey:key_move_speed]intValue];
        self.rotSpeed = [[props objectForKey:key_rot_speed]intValue];
        self.rotation = self.startRotation = [[props objectForKey:key_rotation]floatValue];
        self.appearingDelay = [[props objectForKey:key_appearing_delay]floatValue];
        
        _dirX = self.position.x;
        _dirY = self.position.y;
        
        _particle = [CCParticleSystemQuad particleWithFile:@"smoke.plist"];
        _particle.position = ccp(0,-50);
        [self addChild:_particle];
        [_particle autoRemoveOnFinish];
        
        [self schedule:@selector(createSmoke) interval:0.1];
...
-(void) createSmoke{
    if ([self getActionByTag:move_tag] || [self getActionByTag:rot_tag]) {
        _particle.visible = YES;
        [_particle resetSystem];
    }else{
        _particle.visible = NO;
    }
...
-(void) rotateWithAngle: (float) angle moveAndCallBlock: (void(^)()) block{
    ...
    float rotDuration = differAngle / self.rotSpeed;
    float moveDuration = ccpDistance(self.position,ccp(_dirX, _dirY))/ self.moveSpeed;
...
-(void) shootFromX: (float) x{
    NSDictionary* dict = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:self.shotDistance],key_shot_distance,
                          [NSNumber numberWithFloat:self.rotation],key_rotation,
                          [NSValue valueWithCGPoint:[self convertToWorldSpace:ccp(x, 55)]], key_position ,
                          [NSNumber numberWithInt:self.damage],key_damage,nil];
