...
@property (strong, nonatomic) CCMenu* pauseMenu;
@end
...
-(id)initWithLevel: (int)level
{
    if (self == [super init]) {
        ...
        for (NSDictionary* dict in enemies) {
            ...
            [self createEnemyWithDProperties:dict];
...
-(CGRect) getRectFromObject: (JTObject*) object{
    ...
    NSLog(@"%f",object.sprite.contentSize.height);
...
-(void) showBattleResult: (NSString*) string{
    
    CCLabelTTF* gameOverLabel = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"%@", string] fontName:@"Verdana" fontSize:60];
...
-(void) createPlayer{
    ...
    if (_lives >=0) {
        ...
        _playerTank.position = ccp(500, 400);
